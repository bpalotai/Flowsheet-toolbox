...
def get_case_info(case_id):
    ...
    # Get case parameters - handle both string and dict formats
    if case.parameters:
        if isinstance(case.parameters, dict):
            case_parameters = case.parameters
        else:
            case_parameters = json.loads(case.parameters)
    else:
        case_parameters = {}
...
def run_calibration():
    ...
    try:
        ...
        config = json.loads(surrogate.config) if isinstance(surrogate.config, str) else surrogate.config
        is_inverse = config.get('is_inverse', False)
        ...
        if os.path.isabs(surrogate.model_path):
            # It's a full path
            model_dir = surrogate.model_path
        else:
            # It's a relative path, construct the full path
            model_dir = os.path.join('Cases', case.name, 'ModelFitting', surrogate.name)

        # Check if model.joblib exists directly in this folder
...
@calibration_bp.route('/delete_calibration/<int:case_id>/<timestamp>', methods=['POST'])
def delete_calibration(case_id, timestamp):
    """Delete a calibration result"""
    case = Case.query.get_or_404(case_id)
    file_path = os.path.join('Cases', case.name, "CalibrationResults", f"calibration_results_{timestamp}.xlsx")
    ...
    try:
        # Delete the file
        os.remove(file_path)
        return jsonify({"success": True, "message": "Calibration result deleted successfully"})
    except Exception as e:
        return jsonify({"error": f"Error deleting calibration result: {str(e)}"}), 500
